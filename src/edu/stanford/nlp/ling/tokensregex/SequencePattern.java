...
public class SequencePattern<T> {
  ...
  public static class Env {
    ...
    Class sequenceMatchResultExtractor;
    Class stringMatchResultExtractor;
    ...
    public void bindStringRegex(String var, String regex)
    {
      ...
      stringRegexVariables.put(var, new Pair<Pattern, String>(Pattern.compile(Pattern.quote(var)),regex));
    }
  ...
  protected static class AndPatternExpr extends PatternExpr {
    List<PatternExpr> patterns;
  ...
  protected static class State {
    ...
    protected <T> boolean match(int bid, SequenceMatcher.MatchedStates<T> matchedStates, boolean consume)
    {
      ...
      if (next != null) {
        ...
        for (State s:next) {
          i++;
          boolean m = s.match(matchedStates.branchStates.getBranchId(bid,i,next.size()), matchedStates, consume);
          if (m) {
  ...
  protected static class ConjMatchStateInfo
  {
   ...
   */
  protected static class ConjStartState extends State {
  ...
  /**
   * State for matching the end of a conjuction
   */
  protected static class ConjEndState extends State {
