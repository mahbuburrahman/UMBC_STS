...
public class LexicalizedParserQuery {
  ...
  public boolean parse(List<? extends HasWord> sentence) {
    ...
    if (op.wordFunction != null) {
      ...
      for (int i = 0; i < sentence.size(); ++i) {
        originalWords.add(sentence.get(i).word());
        sentence.get(i).setWord(op.wordFunction.apply(sentence.get(i).word()));
      }
    }
  ...
  private void restoreOriginalWords(List<? extends HasWord> sentence) {
    ...
    Iterator<String> wordIterator = originalWords.iterator();
    while (wordIterator.hasNext()) {
      sentenceIterator.next().setWord(wordIterator.next());
    }
  }
  ...
  private void restoreOriginalWords(Tree tree) {
    ...
    Iterator<String> wordIterator = originalWords.iterator();
    while (wordIterator.hasNext()) {
      leafIterator.next().setValue(wordIterator.next());
    }
  }
