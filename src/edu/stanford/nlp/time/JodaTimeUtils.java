...
public class JodaTimeUtils {
  ...
  public static String timexDateValue(ReadableDateTime begin, ReadableDateTime end){
    return timexDateValue(begin,end,false);
  }
   ...
   * @param forceDate If true, give a "best guess" of the right date; if false, backoff to giving a duration
   *                  for malformed dates.
   * @return The string representation of a DATE type Timex3 expression
   */
  public static String timexDateValue(ReadableDateTime begin, ReadableDateTime end, boolean forceDate){
    //--Special Cases
...
//    System.out.println("  monthDiff=" + monthDiff + "("+maximumValue(dayOfMonth(),begin)+") weekDiff=" + weekDiff + " dayDiff=" + dayDiff + " hrDiff=" + hrDiff + " minDiff=" + minDiff + " dow=" + begin.getDayOfWeek() + " dow_end=" + end.getDayOfWeek());
    //--Years
    if(noFurtherFields(year(), begin, end)){
      int diff = end.getYear()-begin.getYear();
      if(diff == 100 && (forceDate || begin.getYear() % 100 == 0)){
        //(case: century)
        value.append(( begin.getYear() / 100)).append("XX");
      } else if(diff == 10 && (forceDate || begin.getYear() % 10 == 0)){
        //(case: decade)
        value.append(( begin.getYear() / 10));
      } else if(diff == 1 || forceDate){
        //(case: year)
    ...
    } else if(monthDiff < 12 || forceDate) {
      //(case: year and more)
      value.append(begin.getYear());
    } else {
    ...
    if(noFurtherFields(monthOfYear(), begin, end) || noFurtherFields(weekOfWeekyear(), begin, end)){
      ...
      } else if(monthTerminal && monthDiff == 1 && weekDiff != 1 || forceDate) {
        //(case: a month)
      ...
      return value.toString();
    } else if(dayDiff < maximumValue(dayOfMonth(),begin) || forceDate) {
      //(case: month and more)
      value.append(zeroPad(begin.getMonthOfYear(),2));
    } else {
      //(case: treat as duration)
   ...
   * @param approximate If true, treat this period as an approximate duration
   * @return The string representation of a DURATION type Timex3 expression
   */
  public static String timexDurationValue(ReadablePeriod duration, boolean approximate){
    StringBuilder b = new StringBuilder().append("P");
    boolean seenTime = false;
    //(millenia)
    if(duration.get(years()) >= 1000){
      b.append(approximate ? "X" : duration.get(years()) / 1000).append("L");
    }
    //(centuries)
    if(duration.get(years()) >= 100 && duration.get(years()) % 1000 > 0){
      b.append(approximate ? "X" : (duration.get(years()) % 1000)/ 100).append("C");
    }
    //(decades)
    if(duration.get(years()) >= 10 && duration.get(years()) % 100 > 0){
      b.append(approximate ? "X" : (duration.get(years()) % 100) / 10).append("E");
    }
    //(years)
    if(duration.get(years()) % 10 != 0){
      b.append(approximate ? "X" : duration.get(years()) % 10).append("Y");
    }
    //(months)
    if(duration.get(months()) != 0){
//      if((duration.get(months()) % 3) == 0){ //<-- for some reason, this is especially rare in Timebank... --Gabor
//        b.append(approximate ? "X" : duration.get(months()) / 3).append("Q");
//      } else {
        b.append(approximate ? "X" : duration.get(months())).append("M");
//      }
    }
    //(weeks)
    if(duration.get(weeks()) != 0){
      b.append(approximate ? "X" : duration.get(weeks())).append("W");
    }
    //(days)
    if(duration.get(days()) != 0){
      b.append(approximate ? "X" : duration.get(days())).append("D");
    }
    //(hours)
    if(duration.get(hours()) != 0){
      if(!seenTime){ b.append("T"); seenTime = true; }
      b.append(approximate ? "X" : duration.get(hours())).append("H");
    }
    //(minutes)
    if(duration.get(minutes()) != 0){
      if(!seenTime){ b.append("T"); seenTime = true; }
      b.append(approximate ? "X" : duration.get(minutes())).append("M");
    }
    //(seconds)
    if(duration.get(seconds()) != 0){
      if(!seenTime){ b.append("T"); seenTime = true; }
      b.append(approximate ? "X" : duration.get(seconds())).append("S");
    }
    return b.toString();
  }
  public static String timexDurationValue(ReadablePeriod duration){ return timexDurationValue(duration, false); }
